...
def load(control_lib_path,dobotId=0, split="_"):
    ...
    #     require = getRequiredPath("libDobotDll.so")
        print(control_lib_path)
        return CDLL(control_lib_path)
...
def GetDeviceTime(api):
    deviceTime = c_uint32(0)
    while True:
        result = api.GetDeviceTime(byref(deviceTime))
        if result != DobotTypes.DobotCommunicate.DobotCommunicate_Successfully:
            dSleep(5)
            continue
        break
...
def GetAlarmsState(api, maxLen):
    alarmsState = (c_uint8 * maxLen)()
    len_ = c_uint32(0)
    result = api.GetAlarmsState(alarmsState, byref(len_), maxLen)
    if result != DobotTypes.DobotCommunicate.DobotCommunicate_Successfully:
        raise RuntimeError(f"GetAlarmsState failed with error code: {result}")
    if len_.value>0:
        alarms_list = [alarmsState[i] for i in range(len_.value)]
        # 将十进制数转换为8位二进制字符串并逆序
        reversed_binary_list = [bin(x)[2:].zfill(8)[::-1] for x in alarms_list]
        # 计算所有值为1的位的索引
        indices_list = [hex(i * 8 + j) for i, binary_num in enumerate(reversed_binary_list) for j, bit in enumerate(binary_num) if bit == '1']
        return indices_list
    else:
        return []
...
def SetHOMECmd(api, reserved, isQueued=0):
...
def GetEndEffectorSuctionCup(api):
    ...
    isSucked = c_int(0)
    while True:
        result = api.GetEndEffectorSuctionCup(byref(enableCtrl), byref(isSucked))
        if result != DobotTypes.DobotCommunicate.DobotCommunicate_Successfully:
            dSleep(5)
            continue
        break
    #output('GetEndEffectorSuctionCup: isOn=%.4f' % isOn.value)
    return [enableCtrl.value,isSucked.value]
...
def GetEndEffectorGripper(api):
    ...
    isGripped = c_int(0)
    while True:
        result = api.GetEndEffectorGripper(byref(enableCtrl), byref(isGripped))
        if result != DobotTypes.DobotCommunicate.DobotCommunicate_Successfully:
            dSleep(5)
            continue
        break
    #output('GetEndEffectorGripper: isOn=%.4f' % isOn.value)
    return [enableCtrl.value,isGripped.value]
